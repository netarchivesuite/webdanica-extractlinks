...
public class ExtractLinks { 
     ...
     * @param onlyUrlsPointingOutsideDK 
     * @return a set of urls referring outside the .DK
     * @throws ParserException 
     */
    public Set<String> getOutLinks(boolean onlyUrlsPointingOutsideDK) throws ParserException {
    	NodeFilter filter;
    	NodeList list;
    	...
    	try {
    		...
    		for (int i = 0; i < list.size (); i++) {
    			...
    			// add url, if url not null and outside .dk
    			if (url == null) {
    				continue;
    			} else if (url.startsWith("#")) { // Ignore anchor links
    				continue;
    			} else if (url.startsWith("/")) { // Ignore relative links
    				continue;
    			}

    			if (onlyUrlsPointingOutsideDK) {
    				if(pointsOutsideDK(url)) links.add(url);
